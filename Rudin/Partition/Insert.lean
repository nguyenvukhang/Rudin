...
private noncomputable def ι (x : Ioo a b) (P : Partition I) : Partition I
  := by --
...
noncomputable instance : Insert (Ioo a b) (Partition I)
  := by --
  exact { insert x P := ι x P } -- ∎

theorem insert_list_of_notMem {x : Ioo a b} :
  ↑x ∉ P → (insert x P).l = P.l.orderedInsert (· ≤ ·) ↑x
  := by --
  intro hxP
